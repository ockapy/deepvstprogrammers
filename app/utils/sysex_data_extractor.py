...
class DX7globals:
    ...
    def _decodeVMEM(self,data):
        ...
        self.pModSens = (data[116] >> 4) # TODO: resolove operation
...
class VMEM:
    ...
    def _analyse(self,data):
        ...
        # On range dans une liste les bytes voices
        voiceData = sx[6:datasize+6]
        
        offset = 134

        # On range dans la liste les 31 autres voices, avec 128 octets de décalage à chaque itération
        for i in range(32):
            voice = voiceData[(i*128):((i+1)*128)]
            self.voices.append(DX7voice(voice,True))
