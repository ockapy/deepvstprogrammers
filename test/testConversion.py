...
def encode_Operator(operator):
    ...
    break_point = round(operator[8] * 99)
    l_scale_depth = round(operator[9] * 99)
    r_scale_depth = round(operator[10] * 99)
    l_key_curve = round(operator[11]*3)
    r_key_curve = round(operator[12]*3)
    detune = round(operator[13] * 14)
    osc_rate_scale = round(operator[14] * 7)
    key_vel_sens = round(operator[15] * 7)
    amp_mod_sens = round(operator[16] * 3)
    output_level = round(operator[17] * 99)
    freq_coarse = round(operator[18] * 31)
    osc_mode = round(operator[19])
    freq_fine = round(operator[20] * 99)
...
def encode_voice(voice):
    ...
    lpms_lfw_lks_byte =  ((lfo_key_sync & 0b1) | ((lfo_wave & 0b1111) << 1) | ((lfo_pm_sens & 0b11) << 5))
...
patch = [0.5 for _ in range(155)]
data = transform_to_voice(patch=patch)

with open('output.syx','wb') as file:
    file.write(data)
print ("data written")